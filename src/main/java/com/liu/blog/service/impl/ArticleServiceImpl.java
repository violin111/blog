...
import com.liu.blog.enums.FileExtEnum;
...
import static com.liu.blog.constant.RedisPrefixConst.ARTICLE_LIKE_COUNT;
import static com.liu.blog.constant.RedisPrefixConst.ARTICLE_VIEWS_COUNT;
import static com.liu.blog.enums.ArticleStatusEnum.DRAFT;
...
public class ArticleServiceImpl extends ServiceImpl<ArticleDao,Article>implements ArticleService {
    ...
    public PageResult<Article> listRArticle(){
        Page<Article> page = new Page<>(PageUtils.getCurrent(), PageUtils.getSize());
        articleDao.selectPage(page,new LambdaQueryWrapper<Article>()
    ...
    public void saveOrUpdateArticle(ArticleVO articleVO) {
        ...
        //WebsiteConfigVO webConfig = blogInfoService.getWebsiteConfig();
        //FutureTask<WebsiteConfigVO> webConfig = new FutureTask<>(new Callable<WebsiteConfigVO>() {
           // @Override
           // public WebsiteConfigVO call() throws Exception {
              //  return blogInfoService.getWebsiteConfig();
           // }
        //});
        //保存文章分类
        Category category = saveArticleCategory(articleVO);
        //CompletableFuture<Category> category = CompletableFuture.supplyAsync(() -> saveArticleCategory(articleVO));
        //保存或者修改文章
        ...
        if(StringUtils.isBlank(article.getArticleCover())){
            try {
                ...
                throw new BizException("设定默认文章封面");
            }
        }
        //设置文章用户id
    ...
    public void updateArticleDelete(DeleteVO deleteVO) {
        //修改文章逻辑删除状态
        this.updateBatchById(deleteVO.getIdList().stream().map(item->
                Article.builder()
                        .id(item)
                        .isTop(FALSE)
    ...
    /**
     * 这里是首先将文章的信息存放在一个list里面然后将文章内容存放到阿里云存储中就可以下载了
     * @param articleIdList
     * @return
     */
    @Override
    public List<String> exportArticles(List<Integer> articleIdList) {
        ...
        for (Article article : articleList) {
            try (ByteArrayInputStream inputStream = new ByteArrayInputStream(article.getArticleContent().getBytes())) {
                ...
                StrUtil.format("导入文章失败,堆栈:{}", ExceptionUtil.stacktraceToString(e));
    ...
    @Override
    public void updateArticleTop(ArticleVO articleVO) {
        //封装信息
        Article article = Article.builder().id(articleVO.getId()).isTop(articleVO.getIsTop()).build();
    ...
    private Category saveArticleCategory (ArticleVO articleVO){
        ...
        if(Objects.isNull(category)&&!articleVO.getStatus().equals(DRAFT.getStatus())){
            categoryDao.insert(Category.builder().categoryName(articleVO.getCategoryName()).build());
        }
        return category;
    }
